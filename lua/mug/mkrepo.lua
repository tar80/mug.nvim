...
local function do_mkrepo(root, compared, contain)
  local log, result, loglevel = {}, {}, 2
  local function log_table(title)
    local msg = (#result == 0) and '' or result[1]:gsub('\n$', '')
    table.insert(log, string.format('%s%s', title, msg))
  end
  local function notify()
    util.notify(log, HEADER, loglevel, true)
  end

  job.async(function()
    loglevel, result = job.await(util.gitcmd({ noquotepath = true, cmd = 'init', opts = { root } }))
    ...
    if not _G.Mug.remote_url then
      ...
      loglevel, result = job.await(util.gitcmd({ wd = root, cmd = 'remote', opts = { 'add', 'origin', url } }))

      if loglevel then
        result = {}
        log_table(string.format('(remote-add)success: origin %s', url))
      else
    ...
    if contain then
      loglevel, result = job.await(util.gitcmd({ wd = root, cmd = 'add', opts = { '.' } }))
    ...
    if loglevel == 2 then
      ...
      if compared == 'differ' then
        if util.interactive('Change "lcd" to the repository path you created?', HEADER, 'y') then
          vim.cmd.lcd(root)
        end
      end
    else
...
vim.api.nvim_create_user_command('MugMkrepo', function(opts)
  ...
  if vim.fn.isdirectory(repo_root .. '/.git') ~= 0 then
    return util.notify('Already exist', HEADER, 2)
  end
